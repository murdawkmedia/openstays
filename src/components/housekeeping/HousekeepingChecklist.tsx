...
export function HousekeepingChecklist(props: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-stone-950/30" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) props.onClose(); }}>
      <aside ref={dialog.dialogRef} tabIndex={-1} onKeyDown={dialog.onKeyDown} className="h-full w-full max-w-xl overflow-y-auto bg-white p-5 shadow-2xl focus:outline-none" role="dialog" aria-modal="true" aria-labelledby="housekeeping-work-title">
