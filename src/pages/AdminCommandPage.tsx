...
const ACTIONS = ['Reserve', 'Block', 'Quote', 'Repair', 'Call', 'Complimentary', 'Retail'] as const;
    ...
    <div className="space-y-5">
        ...
        <div className="flex flex-wrap gap-2">
          {ACTIONS.map((action) => (
            <button
              key={action}
              type="button"
              className="rounded-lg border border-stone-300 bg-white px-4 py-2 text-sm font-semibold text-stone-800 shadow-sm hover:bg-stone-50 disabled:cursor-not-allowed disabled:opacity-45"
              disabled={!commandCenterEnabled}
              title={!commandCenterEnabled ? 'Enable the accepted workflow for this property first' : `${action} workflow`}
            >
              {action}
            </button>
          ))}
      ...
      {selectedBooking ? (
        <aside className="fixed inset-y-0 right-0 z-50 w-full max-w-md overflow-y-auto border-l border-stone-200 bg-white p-6 shadow-2xl" aria-label="Selected reservation" aria-live="polite">
          <div className="flex items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-wide text-emerald-700">Reservation</p><h2 className="mt-1 text-xl font-semibold">{selectedBooking.confirmationCode}</h2></div><button type="button" className="rounded-lg p-2 hover:bg-stone-100" aria-label="Close reservation" onClick={() => setSelectedBookingId(null)}><X className="h-5 w-5" /></button></div>
          ...
          <div className="mt-6 grid grid-cols-2 gap-2">{['Move', 'Extend', 'Shorten', 'Adjust rate'].map((action) => <button key={action} type="button" className="btn-secondary" disabled={!commandCenterEnabled}>{action}</button>)}</div>
        </aside>
      ) : null}
    </div>
  );
