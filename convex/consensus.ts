...
export function buildConsensusTimeline(input: TimelineInput): ConsensusStep[] {
  ...
  const cancelled = input.statusHistory.includes('cancelled');
  ...
  return [
    ...
    { key: 'confirmed', label: cancelled && confirmed ? 'Booking was confirmed' : 'Booking confirmed', state: confirmed ? 'reached' : 'pending', detail: confirmed ? cancelled ? 'This historical milestone is preserved; the booking is now cancelled.' : 'Payment and availability agree on one booking state.' : 'Confirmation waits for payment and availability consensus.' },
    ...
    { key: 'rewarded', label: 'Guest consensus reward', state: input.rewardStatus === 'paid' ? 'reached' : input.rewardStatus === 'failed' ? 'attention' : input.rewardStatus ? 'pending' : 'ready', detail: input.rewardStatus === 'paid' ? 'The guest wallet received exactly 1,000 signet sats.' : input.rewardStatus === 'paying' || input.rewardStatus === 'invoice_ready' ? 'The merchant bridge is reconciling the 1,000-sat payout.' : input.rewardStatus === 'eligible' ? 'The submitted receipt unlocked a one-time 1,000-sat signet reward.' : input.rewardStatus ? 'The reward remains recorded but has not settled.' : 'No signet reward is attached to this booking.' },
